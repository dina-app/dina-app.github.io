...
- [x] Resolved the open question about the homepage marks by checking what each product actually ships. Admin Toolkit and SheetConnect ship the three-tone fan and now carry it here; Agent, Sheet, Dock, DevOps, Prompter and Voice still ship the neon D and keep it. The homepage mixes because the product family currently mixes, so each card matches its own product. Revisit when the remaining six move.
