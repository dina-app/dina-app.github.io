...
- [x] **Deleted `特定商取引法に基づく表記`.** The disclosure only applies to businesses that sell, so with nothing sold there was no page to keep. Its URL was published and in the sitemap, so `404.html` redirects it to the Agent page rather than letting it 404; the sitemap entry, both links from the Agent page, and the link from its privacy policy are gone, along with 40 lines of disclosure-table CSS. Site validation now fails if the file reappears, if anything links to it again, or if the redirect is dropped.
- [ ] If paid sales are ever reintroduced, republish that page **before** any checkout goes live, with seller identity, address, phone, email, price, payment timing, delivery timing, cancellation terms and refund policy, and remove the redirect and the guards added for its removal.
...
- [x] **Took SheetConnect off the green mark on September 15, 2026.** Google refused it in OAuth verification — "your logo does not uniquely identify your brand and identity ... does not impersonate any other brand" — because a green rounded square carrying a spreadsheet grid reads as Google Sheets. This page is the application home page named in that verification, so it was showing the refused mark at the exact URL the reviewer opens. `logo.svg` resynced to the gold fan with the exchange glyph, `--app-accent` for the third product card moved off `#0a9256`, the `sync-brand-marks.mjs` entry corrected from emerald/spreadsheet-grid to gold/exchange-arrow, and all five product screenshots re-copied now that the panel's accents changed with it.
- [x] **Published `apps/sheetconnect-for-salesforce/picker.html`.** Google declined the `spreadsheets` scope and asked for `drive.file`, which only grants files the user hands over through the Google Picker. The Picker is remote code from `apis.google.com` and Manifest V3 forbids an extension loading remote code, so it cannot run in the side panel — it runs on this page and reports the granted file back over `externally_connectable`. The page is inert until the extension opens it: it carries no content of its own and is `noindex`. Its API key is restricted to the Google Picker API and an `https://dina.jp/*` referrer.
- [x] **Corrected the release claims on September 15, 2026.** The site said SheetConnect was a release candidate "not yet on the Chrome Web Store" while the item was published at 0.4.3 — on the page named as the application home page in Google's OAuth verification. Three places said three different versions (0.3.0 hero and meta description, 0.1.0 homepage card and current-focus board). All now read **Published · 0.4.3**, with an Install free link on the product page and the homepage card, which every other published app already had. The meta description no longer carries a version at all, because it is the one that drifted furthest without anyone noticing.
- [x] Rewrote the Release status section, which claimed live sign-in, import and CRUD were untested — they were run end to end against a real org on September 14 — and stated the scope narrowing from `spreadsheets` to `drive.file` as what it is for a reader: less access, not more. It now also says plainly that the unverified-app screen appears and that authorizations are capped until Google's review completes.
- [ ] Re-read this section when 0.5.0 uploads; the version appears in four places on two pages.
...
- [x] Replace Sheet for Salesforce's generated neon-D homepage mark with its canonical transparent dimensional green D and spreadsheet from `DinaSheet for Salesforce Store Assets/brand-mark-master.png`. The sync script copies it intact and checks drift; the neon generator no longer owns it. This uses its dedicated master, not SheetConnect's fan entry. Site change is local until published.
