(function () {
  ...
  var translations = {
    ...
    "Admin Toolkit for Salesforce is an independent Chrome right-side panel suite compatible with Salesforce. It uses the Salesforce session you are already signed in with for metadata administration, spreadsheet data work, permission analysis, object schema exploration, GitHub metadata retrieval, API tools, Apex execution, exports, and monitoring without a developer-operated metadata service.": "Admin Toolkit for Salesforce は Salesforce と互換性のある独立した Chrome 右サイドパネルスイートです。現在ログイン中の Salesforce セッションを利用し、開発者運営のメタデータサービスを介さずに、メタデータ管理、スプレッドシートでのデータ作業、権限分析、オブジェクトスキーマ探索、GitHub メタデータ取得、API ツール、Apex 実行、エクスポート、モニタリングを提供します。",
    ...
    "Metadata Admin for Salesforce": "Salesforce メタデータ管理",
    "Browse Salesforce objects and reports in a multi-tab spreadsheet grid, inspect file details, and review exact changed fields before confirmed record updates or deletes.": "複数タブのスプレッドシートグリッドで Salesforce オブジェクトやレポートを参照し、ファイル詳細を確認して、レコード更新または削除を確定する前に変更項目を正確にレビューできます。",
    "GitHub Metadata Sync": "GitHub メタデータ同期",
    "Configure and preflight governed metadata retrieval to an approved repository through an org-side bridge, Salesforce native credentials, and a pull-request-only delivery workflow.": "組織側ブリッジ、Salesforce ネイティブ認証情報、プルリクエスト限定の配信ワークフローを通じて、承認済みリポジトリへの統制されたメタデータ取得を設定・事前検証します。",
    "Org Review": "組織レビュー",
    ...
    "Permission Explorer": "権限エクスプローラー",
    "Explain why a user can access an object or field, identify overlapping grants, and trace each permission to the profile, permission set, or permission set group that grants it.": "ユーザーがオブジェクトや項目にアクセスできる理由を説明し、重複する付与を特定して、各権限を付与元のプロファイル、権限セット、または権限セットグループまで追跡します。",
    ...
    "Trace object and field permissions to the profiles, permission sets, and permission set groups that grant them.": "オブジェクトと項目の権限を、付与元のプロファイル、権限セット、権限セットグループまで追跡します。",
    "Configure and validate a governed, allowlisted Salesforce-to-GitHub metadata retrieval workflow without storing GitHub tokens in the browser.": "GitHub トークンをブラウザーに保存せず、許可リストで統制された Salesforce から GitHub へのメタデータ取得ワークフローを設定・検証します。",
    ...
    "Browser API requests containing Salesforce data, identity data, authentication information, and user-entered requests are sent only to the selected Salesforce org, not to developer-operated servers.": "Salesforce データ、識別情報、認証情報、ユーザー入力を含むブラウザー API リクエストは、選択した Salesforce 組織のみに送信され、開発者運営のサーバーには送信されません。",
    "When an administrator explicitly configures and a user confirms GitHub Metadata Sync, the org-side bridge and protected workflow may retrieve only an approved metadata scope into the configured organization-owned GitHub repository through a branch and pull request.": "管理者が明示的に設定し、ユーザーが GitHub メタデータ同期を確認すると、組織側ブリッジと保護されたワークフローは、承認されたメタデータ範囲だけをブランチとプルリクエスト経由で組織所有の設定済み GitHub リポジトリへ取得できます。",
    "Workspace preferences, search data, pending edits, saved SheetD files, recent/favorite tools, sanitized operation history, and non-secret organization-bound GitHub Metadata Sync drafts may be stored locally in Chrome browser storage on the user's device.": "ワークスペース設定、検索データ、保留中の編集、保存済み SheetD ファイル、最近使用したツール・お気に入り、サニタイズ済み操作履歴、組織に紐づく非機密の GitHub メタデータ同期ドラフトは、ユーザー端末の Chrome ストレージにローカル保存される場合があります。",
    ...
    "Read the full": "全文を読む:",
    "Read the full Privacy Policy": "プライバシーポリシーの全文をご確認ください",
    "Read the": "次を確認:",
    ...
    "Install Admin Toolkit for Salesforce from the Chrome Web Store": "Chrome ウェブストアから Admin Toolkit for Salesforce をインストールします",
    ...
    "Version 0.8.0 adds GitHub Metadata Sync as the sixth app, with a resumable Setup Center, verified org and server prefill, server-derived technical configuration, governed preflight, Salesforce native Named Credential APIs, and pull-request-only delivery. It also rebuilds SheetD on the DinaSheet engine so objects and reports open directly as editable data and pending cell edits are reviewed from a Changed fields card before a confirmed update. No browser permission was added, and the extension stores no GitHub secret, token, or OAuth URL.": "バージョン 0.8.0 では、再開可能なセットアップセンター、検証済みの組織・サーバー事前入力、サーバー由来の技術設定、統制された事前検証、Salesforce ネイティブの Named Credential API、プルリクエスト限定の配信を備えた 6 番目のアプリ「GitHub メタデータ同期」を追加しました。また SheetD を DinaSheet エンジンで再構築し、オブジェクトやレポートを編集可能なデータとして直接開き、確定更新前に「変更項目」カードから保留中のセル編集を確認できます。ブラウザー権限は追加されず、GitHub のシークレット、トークン、OAuth URL は保存されません。",
    "Support": "サポート",
    ...
    ". Report a problem or ask a product question through the": "。不具合の報告や製品に関する質問は、次のページからお送りください:",
    "Agent for Salesforce is an independent Chrome extension in development for Salesforce administrators and developers. It adds a side-panel assistant that can use selected Salesforce org context and route chat requests through a Cloud Run function backend.": "Agent for Salesforce は、Salesforce の管理者と開発者向けに開発中の独立した Chrome 拡張機能です。選択した Salesforce 組織コンテキストを利用し、Cloud Run function バックエンドを経由してチャットリクエストを処理するサイドパネルアシスタントを追加します。",
    ...
    "Agent for Salesforce separates the browser extension from the OpenAI transport layer.": "Agent for Salesforce はブラウザー拡張機能と OpenAI 通信レイヤーを分離しています。",
    ...
    "Status": "ステータス",
    "Agent for Salesforce is currently in development and is not publicly released.": "Agent for Salesforce は現在開発中で、一般公開されていません。",
    "Common Tools": "共通ツール",
  ...
  var titleTranslations = {
    ...
    "Agent for Salesforce | Compatible with Salesforce": "Agent for Salesforce | Salesforce 対応",
