(function () {
  ...
  var translations = {
    ...
    ".": "。",
    "Current release: 0.7.0": "現在のリリース: 0.7.0",
    "Admin Toolkit for Salesforce is an independent Chrome right-side panel suite compatible with Salesforce. It uses the Salesforce session you are already signed in with to provide metadata administration, SOQL workspaces, permission analysis, object schema exploration, API tools, Apex execution, exports, and monitoring without a developer-operated metadata service.": "Admin Toolkit for Salesforce は Salesforce と互換性のある独立した Chrome 右サイドパネルスイートです。現在ログイン中の Salesforce セッションを利用し、開発者運営のメタデータサービスを介さずに、メタデータ管理、SOQL ワークスペース、権限分析、オブジェクトスキーマ探索、API ツール、Apex 実行、エクスポート、モニタリングを提供します。",
    ...
    "Version 0.7.0 adds Permission Explorer, a read-only workspace that explains why a user can access an object or field. It lists every assigned record that grants access, highlights objects granted by more than one record, and traces each permission to the profile, permission set, or permission set group that grants it. Permission Explorer explains object and field permissions only, not record-level sharing. No new Chrome permissions were added.": "バージョン 0.7.0 では、ユーザーがオブジェクトや項目にアクセスできる理由を説明する読み取り専用ワークスペース「権限エクスプローラー」を追加しました。アクセスを付与するすべての割り当て済みレコードを一覧表示し、複数のレコードから権限を付与されているオブジェクトを強調して、各権限を付与元のプロファイル、権限セット、または権限セットグループまで追跡します。権限エクスプローラーが説明するのはオブジェクト権限と項目権限のみで、レコードレベルの共有は対象外です。新しい Chrome 権限は追加されていません。",
    "Support": "サポート",
