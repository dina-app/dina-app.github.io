...
const ICON = {
  ...
  book: '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M5 4.5A1.5 1.5 0 0 1 6.5 3H19v15H6.5A1.5 1.5 0 0 0 5 19.5zM5 19.5A1.5 1.5 0 0 0 6.5 21H19" stroke="currentColor" stroke-width="1.8" stroke-linejoin="round" fill="none"/><path d="M9 7.5h6M9 11h6" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"/></svg>',
  bug: '<svg viewBox="0 0 24 24" aria-hidden="true"><rect x="7" y="7" width="10" height="13" rx="5" stroke="currentColor" stroke-width="1.8" fill="none"/><path d="M9 7.5a3 3 0 0 1 6 0M12 11v9M4 13h3M17 13h3M5 8l2.5 2M19 8l-2.5 2M5 19l2.5-2M19 19l-2.5-2" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" fill="none"/></svg>',
  idea: '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M9 17h6M10 20.5h4M12 3a6 6 0 0 0-3.6 10.8c.6.5 1 1.3 1 2.2h5.2c0-.9.4-1.7 1-2.2A6 6 0 0 0 12 3z" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" fill="none"/></svg>',
...
function buildPanel() {
  ...
  panel.innerHTML = `
    ...
    <section class="support-gate" data-signed-out hidden>
      ...
      <p class="support-notice" ${both("AI chat messages go to OpenAI, which keeps each conversation for 30 days so it can continue it; DinaLab keeps a record of it for 30 days too. Reports you send are kept with your name and email so we can reply.", "AI チャットのメッセージは OpenAI に送信され、会話を続けるため 30 日間保存されます。DinaLab も会話の記録を 30 日間保管します。送信したご報告は、返信のためにお名前とメールアドレスとともに保管します。")}>${say("AI chat messages go to OpenAI, which keeps each conversation for 30 days so it can continue it; DinaLab keeps a record of it for 30 days too. Reports you send are kept with your name and email so we can reply.", "AI チャットのメッセージは OpenAI に送信され、会話を続けるため 30 日間保存されます。DinaLab も会話の記録を 30 日間保管します。送信したご報告は、返信のためにお名前とメールアドレスとともに保管します。")}</p>
      <p class="support-error" role="alert" data-sign-in-error hidden></p>
    </section>

    <section class="support-body" data-signed-in hidden>
      <div class="support-view" data-support-view></div>
    </section>
...
// Everything that needs the network — sign-in, the app list, the four ways in —
// is in /support/support-panel.js, fetched only when the panel first opens.
async function start() {
  const { mountPanel } = await import("/support/support-panel.js");
  await mountPanel({ panel, part: name => panel.querySelector(`[data-${name}]`), readState, writeState, pageApp, lang, say, ICON, logoFor, toggleMenu });
}
